// @ts-nocheck
/**
 * Script to reset document verification and delete documents for brookechimoto@gmail.com
 * 
 * This will:
 * 1. Set isVerified to true (email/account verification)
...
async function resetBrookeDocuments() {
  try {
    ...
    console.log('🗑️  RESETTING DOCUMENT VERIFICATION FOR BROOKE');
    console.log('═'.repeat(80));
    ...
    // Reset verification and delete all documents
    const result = await db.collection('users').updateOne(
      { email: email.toLowerCase() },
      {
        $set: {
          'isVerified': true, // email/account verification ON
          'isActive': true, // account must be active to login
    ...
    if (result.modifiedCount > 0) {
      console.log('✅ Document verification reset + email verification set to TRUE!\n');
    } else {
    ...
    console.log('');
    console.log('✅ Brooke now has email/account verification = TRUE, documents cleared and set to unverified');
    console.log('═'.repeat(80));
