...
const { addMonths } = require('./src/config/testMode');
...
async function updateRentalForTesting() {
  ...
  try {
    ...
    // Import addMonths from testMode config
    const testModeConfig = require('./src/config/testMode');
    const addMonthsFn = testModeConfig.addMonths;
    ...
    console.log(`   - Second payment due: ${addMonthsFn(newStartDate, 1).toISOString()} (10 minutes)`);
    console.log(`\n🎯 Test timeline:`);
